...
{vDischarge(D, T, P_S ): vGUESS(P_S)} = 1 :- time(I, T), date(D), timeTarget(T).
...
:- time(I, T1), timeTarget(T1), vE_Smin(E_Smin), E_Smin - E_Sinit > #sum{-P_S1, J, 1: vDischarge(D, T2, P_S1), time(J, T2), J <= I; P_S2, J, 2: vCharge(D, T2, P_S2), time(J, T2), J <= I}, vE_Sinit(E_Sinit).
...
recommend(D, T, "dicharge") :- vDischarge(D, T, V), V > 0.
recommend(D, T, "charge") :- vCharge(D, T, V), V > 0.
