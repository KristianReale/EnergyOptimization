...
do    
	...
	date="${date%.*}"
        echo "Executing ${date}"  
        mkdir -p "Results/H${i}"
        /home/dlv/Clingo/clingo-4.5.4-linux-x86_64/clingo encoding_article.asp params_article.asp maxChargeKwh.asp "${factsFile}" "${initChargeFile}" $1 --parallel-mode=8 > "Results/H${i}/output_H${i}_${date}.txt" --time-limit=1200 --quiet=1 --outf=1
	sed -e 's/ANSWER/%ANSWER/g' "Results/H${i}/output_H${i}_${date}.txt" > "Results/H${i}/tmp.txt"
	sed -e 's/COST/%COST/g' "Results/H${i}/tmp.txt" > "Results/H${i}/output_H${i}_${date}.txt"
