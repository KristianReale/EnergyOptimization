    ...
    def test_execution(self):
        for filename in os.listdir('DatasetDati'):
            if filename.endswith('_Wh.csv'):
                ...
                build_from_csv(f"DatasetDati/{filename}", f"InputDataset/{houseFolder}_1_min_Wh/asp",split_data=SPLIT_DATA.DAY, format=FORMAT.ASP, unit="Wh", minute_granularity=1, noRound=True)
                build_from_csv(f"DatasetDati/{filename}", f"InputDataset/{houseFolder}_1_min_Wh/csv",split_data=SPLIT_DATA.DAY, format=FORMAT.CSV, unit="Wh", minute_granularity=1, noRound=True)
