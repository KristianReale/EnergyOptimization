...
def group_data(input_file, minute_granularity=60): #per media
    ...
    with open(input_file, 'r') as csvfile:
        ...
        for row in reader:
            ...
            # date, true_cons, dLinear, TimesNet, true_prod, prod = row
            hourly_data[key]['true_cons'].append(float(true_cons))
            hourly_data[key]['dLinear'].append(float(dLinear))
            hourly_data[key]['TimesNet'].append(float(TimesNet))
    ...
    for house in house_list:
        ...
        for file in file_list:
            ...
            input_file_csv = os.path.dirname(
                os.path.abspath(__file__)) + "/InputDataset" + f"/{house}/csv/{date_file}.csv"

            ws1 = wb.create_sheet(title=date_file)
            init_state_of_charge = 10000
